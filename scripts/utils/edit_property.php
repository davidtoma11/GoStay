...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['delete_photo_id']) && !isset($_POST['confirm_delete_property'])) {
    try {
        ...
        $stmt_upd = $db->prepare("UPDATE rooms SET city_id=?, name=?, description=?, address=?, price=?, capacity=?, bedrooms=?, bathrooms=?, check_in_time=?, check_out_time=? WHERE id=?");
        $stmt_upd->execute([
            $_POST['city_id'], $_POST['name'], $_POST['description'], $_POST['address'], 
            $_POST['price'], $_POST['capacity'], $_POST['bedrooms'], $_POST['bathrooms'], 
            $_POST['check_in_time'], $_POST['check_out_time'], $roomId
        ]);
...
<body style="background: #f4f7ff;">
    <nav class="results-nav">
        ...
        <div class="nav-icons">
            <a href="../pages/manager_dashboard.php"><i class="fa-solid fa-arrow-left"></i></a>
        </div>
    </nav>
            ...
            <div class="manager-card-section">
                ...
                <?php if(isset($_GET['success'])): ?>
                    <div style="background: #d1fae5; color: #065f46; padding: 15px; border-radius: 12px; margin-bottom: 20px;">
                        ...
                        <?php 
                            if($_GET['success'] == 'updated') echo "Changes saved successfully";
                ...
                <form method="POST" enctype="multipart/form-data" id="mainEditForm">
                    ...
                    <div class="form-grid-layout">
                        <div class="input-group">
                            ...
                            <input type="text" name="name" value="<?= htmlspecialchars($roomData['name']) ?>" required>
                        </div>
                        <div class="input-group">
                            <label>City</label>
                            <select name="city_id">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="price" value="<?= $roomData['price'] ?>" required>
                        </div>
                    ...
                    <div class="preview-container" style="margin-bottom: 20px;">
                        <?php foreach($roomPhotos as $p): ?>
                            <div style="position:relative; width: 120px; height: 120px;">
                                ...
                                <button type="submit" name="delete_photo_id" value="<?= $p['id'] ?>" 
                                        style="position:absolute; top:-5px; right:-5px; background: #e74c3c; color: white; border: none; border-radius: 50%; width: 22px; height: 22px; cursor: pointer; display: flex; align-items: center; justify-content: center;">
                                    <i class="fa-solid fa-xmark" style="font-size: 10px;"></i>
                                </button>
                                <?php if($p['is_primary']): ?>
                                    <span style="position:absolute; bottom:5px; left:5px; background: #7b2bd4; color:white; font-size:9px; padding:2px 5px; border-radius:4px;">COVER</span>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
                    </div>
                    ...
                    <div class="upload-zone" id="dropZone">
                        ...
                        <p>Drag more photos here to add</p>
                    ...
                    <div class="facilities-container">
                        ...
                        foreach ($fac_list as $key => $details): ?>
                            <label class="fac-checkbox-item">
                                <input type="checkbox" name="<?= $key ?>" <?= $facData[$key] ? 'checked' : '' ?>>
                    ...
                    <div style="margin-top: 40px; display: flex; gap: 20px;">
                        ...
                        <button type="button" class="btn" onclick="openDeleteModal()"
                                style="flex: 1; background: #fff5f5; color: #e74c3c; border: 1px solid #feb2b2;">
    ...
    <div id="deletePropertyModal" class="res-overlay" style="display: none;">
        <div class="res-overlay-content modal-compact">
            ...
            <p style="margin-bottom: 25px; color: #666; line-height: 1.5;">
                Are you sure you want to permanently delete <strong><?= htmlspecialchars($roomData['name']) ?></strong>? This action cannot be undone.
            </p>
            <form method="POST">
                <button type="submit" name="confirm_delete_property" class="btn" style="background: #e74c3c; width: 100%; margin-left: 30px;">YES, DELETE PROPERTY</button>
            </form>
        </div>
    </div>
